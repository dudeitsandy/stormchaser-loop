...
- **URP Render Graph (6.3)**: `RenderGraphSettings.enableRenderCompatibilityMode` is now read-only (returns false). Compatibility Mode is fully removed. All custom render passes must use the Render Graph API. Projects still on Compatibility Mode must migrate before upgrading to 6.3.
- **URP Custom Shaders**: Cannot be auto-converted by the Render Pipeline Converter — must be manually rewritten for URP compatibility.
...
- **Web**: WebGPU support
- **URP Bloom (6.3)**: New Kawase and Dual filtering options for Bloom post-processing — better quality/performance trade-off options
- **DLSS4 (6.3)**: Deep Learning Super Sampling upgraded to DLSS4 Super Resolution (v310.3.0)
- **Render Graph Defrag (6.3)**: Focused updates to Render Graph — defragmentation, performance, pipeline unification
- **Multi-View Rendering (6.3)**: URP now supports up to four views
